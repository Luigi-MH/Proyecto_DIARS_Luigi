...
namespace CapaDatos
{
    public class datEstadosProducto
    {
        private readonly static datEstadosProducto _instancia = new datEstadosProducto();

        private datEstadosProducto()
        { }

        public static datEstadosProducto Instancia
        {
        ...
        {
            SqlCommand cmd = null;
            List<entEstadoProducto> lista = new List<entEstadoProducto>();
            try
            {
                SqlConnection cn = Conexion.Instancia.Conectar();
                cmd = new SqlCommand("spListarEstadosProducto", cn);
                cmd.CommandType = CommandType.StoredProcedure;
                cn.Open();
                SqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    entEstadoProducto estado = new entEstadoProducto();
                    estado.Id_Estado = Convert.ToInt32(reader["id_EstadoProducto"]);
